...
const TrendingSpecialPrd = () => {
  ...
  let content = null;
  const hotspotProducts = retailProducts.slice(0, 2);
  const hotspot1 = hotspotProducts[0];
  const hotspot2 = hotspotProducts[1];
  ...
  if (!isLoading && !isError && retailProducts.length === 0) {
    content = <ErrorMsg msg="No Products found!" />;
  }
  if (!isLoading && !isError && retailProducts.length > 0) {
    const product_items = retailProducts.slice(0, 4);
    content = (
	                ...
	                <div className="tp-special-thumb">
	                  ...
	                  <div className="tp-special-hotspot-item tp-special-hotspot-1">
	                    ...
	                    <div className="tp-special-hotspot-content">
	                      {hotspot1?._id ? (
	                        <Link href={buildProductPath(hotspot1)} className="d-inline-block">
	                          <h3 className="tp-special-hotspot-title">{hotspot1.title}</h3>
	                        </Link>
	                      ) : (
	                        <h3 className="tp-special-hotspot-title">{hotspot1?.title || 'Featured product'}</h3>
	                      )}
	                      <p>{formatPrice(hotspot1?.price) || hotspot1?.tags?.[0] || 'Trending this week'}</p>
	                    </div>
	                  </div>
	                  <div className="tp-special-hotspot-item tp-special-hotspot-2">
	                    ...
	                    <div className="tp-special-hotspot-content">
	                      {hotspot2?._id ? (
	                        <Link href={buildProductPath(hotspot2)} className="d-inline-block">
	                          <h3 className="tp-special-hotspot-title">{hotspot2.title}</h3>
	                        </Link>
	                      ) : (
	                        <h3 className="tp-special-hotspot-title">{hotspot2?.title || 'Featured product'}</h3>
	                      )}
	                      <p>{formatPrice(hotspot2?.price) || hotspot2?.tags?.[0] || 'New arrival'}</p>
	                    </div>
	                  </div>
