    ...
    <div className="tp-product-list-item d-md-flex">
      <div className="tp-product-list-thumb p-relative fix">
        <Link href={productPath}>
          <Image src={img} alt={imageAlt} width={350} height={310} />
        </Link>
        ...
        <div className="tp-product-content-2 pt-15">
          ...
          <div className="tp-product-price-wrapper-2">
            {purchasable && bogoOffer ? (
              <>
                <span className="tp-product-price-2 new-price">PKR {bogoOffer.bundlePrice.toFixed(2)}</span>
                <span className="tp-product-price-2 old-price"> {bogoOffer.bundleSize} pcs bundle</span>
              </>
            ) : purchasable && discount > 0 ? (
              <>
                <span className="tp-product-price-2 new-price">PKR {price}</span>
                <span className="tp-product-price-2 old-price">
                  {" "}PKR {(Number(price) - (Number(price) * Number(discount)) / 100).toFixed(2)}
                </span>
              </>
            ) : purchasable ? (
              <span className="tp-product-price-2 new-price">PKR {price}</span>
          ...
          {purchasable ? (
            ...
            <InquiryCTAInline productName={title} />
          ) : null}
        </div>
      </div>
    </div>
